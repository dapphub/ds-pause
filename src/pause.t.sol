...
contract Stranger {
    function plan(DSPause pause, address usr, bytes memory fax, uint val, uint era)
        public
    {
        pause.plan(usr, fax, val, era);
    }
    function drop(DSPause pause, address usr, bytes memory fax, uint val, uint era)
        public
    {
        pause.drop(usr, fax, val, era);
    }
    function exec(DSPause pause, address usr, bytes memory fax, uint val, uint era)
        public payable
        returns (bytes memory)
    {
        return pause.exec.value(msg.value)(usr, fax, val, era);
    }
}
...
contract Plan is Test {
    ...
    function testFail_call_from_unauthorized() public {
        ...
        stranger.plan(pause, usr, fax, val, era);
    }
    ...
    function test_plan() public {
        ...
        bytes32 id = keccak256(abi.encode(usr, fax, val, era));
...
contract Exec is Test {
    ...
    function testFail_value_mismatch() public {
        ...
        pause.exec.value(1 ether)(usr, fax, val, era);
    }
    ...
    function test_value_propogation() public {
        ...
        pause.plan(usr, fax, val, era);
        hevm.warp(era);
        bytes memory out = stranger.exec.value(val)(pause, usr, fax, val, era);
...
contract Drop is Test {
    ...
    function testFail_call_from_unauthorized() public {
        ...
        stranger.drop(pause, usr, fax, val, era);
    }
